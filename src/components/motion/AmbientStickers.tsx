 ...
 * stickers — the crown, the wordmark's tally, a star — set faint in the
 * gutters either side of the column, drifting a few pixels on slow
 * transform-only keyframes. These are the platform's own marks, not any one
 * show's: the layout wraps every page, so nothing in it may say Big Brother
 * to someone here for Survivor.
 *
 ...
 * the worst case (the gold crown) at 4.5:1 and above, and the column's tiles
  ...
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 hidden overflow-hidden lg:block">
      ...
      <span className="absolute left-[max(2rem,calc(50vw-32rem))] top-[18%] block -rotate-12 opacity-[0.16]">
        <Doodle kind="crown" className="ambient-shape h-20 w-20 animate-float will-change-transform" />
      </span>
