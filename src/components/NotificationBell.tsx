    ...
    <Link href="/notifications" prefetch={false} aria-label={label} className="icon-btn">
      ...
      {count > 0 && (
        <span
          ...
          aria-hidden
          className="absolute -right-0.5 -top-0.5 grid h-[1.125rem] min-w-[1.125rem] place-items-center rounded-full bg-danger-strong px-1 text-2xs font-bold leading-none text-white ring-2 ring-canvas"
        >
          {count > 99 ? '99+' : count}
        </span>
      )}
