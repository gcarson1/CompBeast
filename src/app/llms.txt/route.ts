...
export async function GET() {
  ...
  const lines = [
    ...
    `> ${SITE_DESCRIPTION}`,
    '',
    `${SITE_NAME} is a web app; it is free to play and runs in a modern web browser. Leagues hold 2 to 24 teams, each team drafts 1 to 12 contestants in a live snake draft, and every league scores with one of its show's rulesets — Classic, Balanced or Drama & Social, plus Lauren’s Way for Big Brother, the house rules of the fan league Comp Beast grew out of. Results for every show are captured from published season results — Big Brother from a fan results site, Survivor and The Traitors from each season's Wikipedia article — reviewed before publishing, and every point traces back to the aired event that produced it.`,
    '',
    '## Start here',
    '',
    ...
    `- [Scoring rules](${base}/rules): every scored event for every show — Big Brother, Survivor, The Traitors — with its point value under each ruleset.`,
